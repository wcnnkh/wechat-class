...
public class ClassController {
	...
	public void downMessage(HttpServletRequest request, ServerHttpResponse httpServletResponse) throws Exception {
		datasource.export(new SimpleSql(
				"select m.openid as 'openid', u.nickName as '昵称', strftime('%Y-%m-%d %H:%m:%s', m.cts) as '时间', m.type as '类型', m.`data` as '内容' from user as u,message as m where u.openid=m.openid"),
